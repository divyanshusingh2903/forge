// Forked from packages/tui keymap for the --mini UI. The full terminal UI
// was removed; --mini keeps registration, base mode, and formatting helpers.
// Palette-only helpers (slash entries, command shortcuts) were dropped.
import {
  InputRenderable,
  TextareaRenderable,
  type CliRenderer,
...
import { KeymapProvider, useKeymapSelector, useBindings } from "@opentui/keymap/solid"
...
export type OpenTuiKeymap = ReturnType<typeof import("@opentui/keymap/solid").useKeymap>
...
function leaderDisplay(config: FormatConfig) {
  ...
  if (!key) return MiniKeybind.LeaderDefault
