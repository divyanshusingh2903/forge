...
describe("layout deep links", () => {
  ...
  test("ignores non-project deep links", () => {
    expect(parseDeepLink("forge://other?directory=/tmp/demo")).toBeUndefined()
    expect(parseDeepLink("https://example.com")).toBeUndefined()
  })

  test("ignores malformed deep links safely", () => {
    expect(() => parseDeepLink("forge://open-project/%E0%A4%A%")).not.toThrow()
    expect(parseDeepLink("forge://open-project/%E0%A4%A%")).toBeUndefined()
  })
  ...
  test("ignores open-project deep links without directory", () => {
    expect(parseDeepLink("forge://open-project")).toBeUndefined()
    expect(parseDeepLink("forge://open-project?directory=")).toBeUndefined()
  })

  test("collects only valid open-project directories", () => {
    const result = collectOpenProjectDeepLinks([
      "forge://open-project?directory=/a",
      "forge://other?directory=/b",
      "forge://open-project?directory=/c",
    ])
    expect(result).toEqual(["/a", "/c"])
  })

  test("parses new-session deep links with optional prompt", () => {
    expect(parseNewSessionDeepLink("forge://new-session?directory=/tmp/demo")).toEqual({ directory: "/tmp/demo" })
  ...
  test("ignores new-session deep links without directory", () => {
    expect(parseNewSessionDeepLink("forge://new-session")).toBeUndefined()
    expect(parseNewSessionDeepLink("forge://new-session?directory=")).toBeUndefined()
  })

  test("collects only valid new-session deep links", () => {
    const result = collectNewSessionDeepLinks([
      "forge://new-session?directory=/a",
      "forge://open-project?directory=/b",
      "forge://new-session?directory=/c&prompt=ship%20it",
    ])
  ...
  test("drains global deep links once", () => {
    const target = {
      __FORGE__: {
        deepLinks: ["forge://open-project?directory=/a"],
      },
    } as unknown as Window & { __FORGE__?: { deepLinks?: string[] } }

    expect(drainPendingDeepLinks(target)).toEqual(["forge://open-project?directory=/a"])
